...
public class ReceivableFinancialCalculator {
    ...
    public void fillLateCharges(Receivable entity, ReceivableDTO dto) {
        BigDecimal amount = valueOrZero(entity.getAmount()).setScale(
                ReceivableConstants.MONEY_SCALE,
                RoundingMode.HALF_UP
        );
        BigDecimal openAmount = getOpenAmount(entity).setScale(
                ReceivableConstants.MONEY_SCALE,
                RoundingMode.HALF_UP
        );
        ...
        dto.setCalculatedLateInterest(ReceivableConstants.ZERO.setScale(
                ReceivableConstants.MONEY_SCALE,
                RoundingMode.HALF_UP
        ));
        dto.setCalculatedLateFee(ReceivableConstants.ZERO.setScale(
                ReceivableConstants.MONEY_SCALE,
                RoundingMode.HALF_UP
        ));
        ...
        amountWithLateCharges = amountWithLateCharges.setScale(
                ReceivableConstants.MONEY_SCALE,
                RoundingMode.HALF_UP
        );
    ...
    private boolean isOverdueOpenReceivable(Receivable entity) {
        ...
        return getOpenAmount(entity).compareTo(ReceivableConstants.ZERO) > 0;
    }
